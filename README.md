...
- Before claim-safe `quality-eval`, refresh the citation evidence surfaces with `paperorchestra audit-rendered-references --quality-mode claim_safe` and `paperorchestra audit-citation-integrity --quality-mode claim_safe`. These commands write session-default `rendered_reference_audit.json`, `citation_intent_plan.json`, `citation_source_match.json`, and `citation_integrity.audit.json`; `quality-eval` records their paths and hashes but still fails closed until an actual `citation_integrity.critic.json` exists.
