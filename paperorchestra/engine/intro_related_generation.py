...
def _repair_intro_related_draft(
    cwd: str | Path | None,
    provider: BaseProvider,
    plan: IntroRelatedPromptPlan,
    ...
    *,
    ...
    for repair_attempt in range(1, max_attempts + 1):
        ...
        retry_draft = _draft_from_response(
            retry_response,
...
def _append_initial_intro_related_notes(plan: IntroRelatedPromptPlan, draft: IntroRelatedDraft) -> IntroRelatedDraft:
...
def _bridge_intro_related_citation_coverage(draft: IntroRelatedDraft, plan: IntroRelatedPromptPlan) -> IntroRelatedDraft:
...
def build_intro_related_draft(
    ...
    *,
    ...
    draft = _repair_intro_related_draft(cwd, provider, plan, draft, runtime_mode=runtime_mode)
    draft = _append_initial_intro_related_notes(plan, draft)
    return _bridge_intro_related_citation_coverage(draft, plan)
