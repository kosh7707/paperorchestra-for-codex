...
from paperorchestra.core.io import write_json as _write_json
from paperorchestra.core.session import artifact_path as _artifact_path, load_session as _load_session
from paperorchestra.loop_engine.quality.loop import (
    DEFAULT_MAX_ITERATIONS,
    write_quality_eval as _write_quality_eval,
    write_quality_loop_plan as _write_quality_loop_plan,
)
...
def build_qa_loop_brief(
    ...
    *,
    ...
    return _build_qa_loop_brief_impl(
        ...
        plan_path=plan_path,
        _load_session_fn=_load_session,
        _artifact_path_fn=_artifact_path,
        _read_json_fn=_read_json,
        _write_quality_eval_fn=_write_quality_eval,
        _write_quality_loop_plan_fn=_write_quality_loop_plan,
...
def write_qa_loop_brief(cwd: str | Path | None, output_path: str | Path | None = None, **kwargs: Any) -> tuple[Path, str]:
    ...
    path = Path(output_path).resolve() if output_path else _artifact_path(cwd, QA_LOOP_BRIEF_FILENAME)
