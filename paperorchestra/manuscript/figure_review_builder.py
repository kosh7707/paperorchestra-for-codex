...
from paperorchestra.manuscript.figure_patterns import CAPTION_RE, FIGURE_ENV_RE, LABEL_RE
...
from paperorchestra.manuscript.figure_review_helpers import (
    bibliography_start_index,
    compact_context,
    figure_references,
    figure_source_origin,
...
from paperorchestra.manuscript.sections import _line_number, _section_for_index, _section_records
...
def _apply_tail_clump_warnings(
...
def build_figure_placement_review(
    ...
    *,
    ...
    for idx, match in enumerate(FIGURE_ENV_RE.finditer(latex), start=1):
        figure, figure_warnings, figure_failures, is_tail_candidate = _figure_record(
            latex=latex,
            match=match,
            idx=idx,
            total_lines=total_lines,
    ...
    _apply_tail_clump_warnings(figures, warnings, tail_figures)
