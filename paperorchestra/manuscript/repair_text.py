...
_MANUSCRIPT_CONTROL_PROSE_REWRITES: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    (re.compile(r"\b(?:supplied|provided)\s+packet\b|\b(?:(?:supplied|provided)\s+)?(?:method|construction|proof|benchmark|empirical|review|source|material)\s+packet\b", re.IGNORECASE), "stated evidence"),
    (re.compile(r"\b(?:supplied|provided)\s+(?:proof|benchmark|empirical|measurement|review)\s+(?:source|logs?)\b", re.IGNORECASE), "stated evidence"),
