...
def build_reproducibility_reasons(
    ...
    *,
    ...
    blocking: list[str] = []
    warnings: list[str] = []
    _append_artifact_blockers(blocking, context)
    _append_live_verification_blockers(blocking, context, require_live_verification=require_live_verification)
    _append_warnings(warnings, context, require_live_verification=require_live_verification)
    _append_strict_content_blockers(blocking, context)
    _append_mock_watermark_warning(warnings, context)
    return ReproducibilityReasons(blocking=blocking, warnings=warnings)


def _append_artifact_blockers(blocking: list[str], context: ReproducibilityAuditContext) -> None:
    state = context.state
    ...
    _append_citation_surface_blocker(blocking, context)


def _append_citation_surface_blocker(blocking: list[str], context: ReproducibilityAuditContext) -> None:
    state = context.state
...
def _append_live_verification_blockers(
    blocking: list[str],
    context: ReproducibilityAuditContext,
    *,
    require_live_verification: bool,
    ...
    if not (require_live_verification and context.verification_invoked and state.latest_verify_mode == "live"):
        return
    _append_live_seed_blocker(blocking, context.citation_live_provenance)
    _append_mixed_provenance_blocker(blocking, context.citation_live_provenance)


def _append_live_seed_blocker(blocking: list[str], citation_live_provenance: dict[str, Any]) -> None:
...
def _append_mixed_provenance_blocker(blocking: list[str], citation_live_provenance: dict[str, Any]) -> None:
...
def _append_strict_content_blockers(blocking: list[str], context: ReproducibilityAuditContext) -> None:
...
def _append_mock_watermark_warning(warnings: list[str], context: ReproducibilityAuditContext) -> None:
    state = context.state
    mock_or_fallback = (
