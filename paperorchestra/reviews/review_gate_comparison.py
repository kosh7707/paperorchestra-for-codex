...
def build_review_gate_payload(*, session_id: str, review_path: str | None, latest_review: Any) -> dict[str, Any]:
    ...
    missing_citation_statistics_keys = (
        [key for key in EXPECTED_CITATION_STATISTICS_KEYS if key not in citation_statistics]
        if isinstance(citation_statistics, dict)
        else EXPECTED_CITATION_STATISTICS_KEYS[:]
    )
    ...
    numeric_axis_scores = _numeric_axis_scores(axis_scores)
    anti_inflation = _anti_inflation_violations(overall_score, numeric_axis_scores)
    return {
