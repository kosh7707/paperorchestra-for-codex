...
from paperorchestra.loop_engine.ralph import repair_issue_packet
from paperorchestra.loop_engine.ralph import repair_recheck_metrics as metrics
...
def test_repair_recheck_metrics_summarize_packet_and_audit_payloads() -> None:
    audit_metrics = metrics._citation_integrity_metrics(
        {
            "status": "fail",
    ...
    packet_metrics = metrics._citation_issue_metrics_from_packet(
        [
    ...
    high_risk = metrics._high_risk_issue_metrics_from_packet(
    ...
    assert metrics._strictly_improves(2, 1) is True
    assert metrics._strictly_improves(2, 2) is False
